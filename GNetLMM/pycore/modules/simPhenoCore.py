...
def simPheno(options):
    ...
    if options.networkArchitecture=='star':
        sim = simulator.HotSpotSimulator(options.bfile)
    elif options.networkArchitecture=='sparse':
        sim = simulator.SparseSimulator(options.bfile)
    else:
        raise Exception("networkArchitecture '%s' is not known."%options.networkArchitecture)
